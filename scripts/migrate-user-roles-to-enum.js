...
function legacyNameToAppRole(name) {
...
async function main() {
    ...
    for (const doc of relations) {
        ...
        const roleId = d.roleId instanceof ObjectId ? d.roleId : new ObjectId(String(d.roleId));
        ...
        await relCol.updateOne({ _id: d._id }, { $set: { role: appRole }, $unset: { roleId: "" } });
        updated++;
    }
    ...
    console.log(`Migration done: updated ${updated}, skipped (already migrated) ${skipped}, duplicates removed ${deduped}`);
    await client.close();
}
main().catch((e) => {
    console.error(e);
...
//# sourceMappingURL=migrate-user-roles-to-enum.js.map